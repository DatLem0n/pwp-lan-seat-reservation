...
public class SeatCreationService {
    ...
    public SeatCreationService(SeatRepository seatRepository, LocationRepository locationRepository) {
    ...
    public void createSeats(SeatCreationDto seatCreationDto) {
        ...
        var lastSeat = seatRepository.findTopByLocationIdOrderBySeatNumberDesc(seatCreationDto.getLocation());
                //.orElseThrow() -> new IllegalArgumentException("No seats in location");

        for (int i = 0; i < seatCreationDto.getSeatCount(); i++) {
            Seat seat = new Seat();
            seat.setSeatNumber(i + lastSeat.getSeatNumber() + 1);
